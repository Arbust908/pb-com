...
export default defineEventHandler(async (event) => {
  try {
    ...
    const existing = await db
      .select()
      .from(cvLanguages)
      .where(eq(cvLanguages.slug, validatedData.slug))
    if (existing.length > 0) {
      throw createError({
        statusCode: 400,
        statusMessage: 'Language with this slug already exists',
      })
    }

    // Get max sortOrder
    const allLanguages = await db.select().from(cvLanguages)
    const maxOrder = allLanguages.reduce((max, l) => Math.max(max, l.sortOrder), -1)
    ...
    const translations = await db
      .select()
      .from(cvTranslations)
      .where(eq(cvTranslations.entitySlug, newLanguage.slug))

    const translationsByLocale: Record<string, Record<string, string>> = {}
    for (const t of translations) {
      if (t.entityType !== 'language') continue
