...
El equipo de SimplyCodes quería tomar decisiones de producto según el comportamiento de los usuarios, no por intuición. Los tests A/B también podían destrabar opiniones contrapuestas: definíamos el comportamiento esperado, comparábamos grupos ante un cambio controlado y usábamos el resultado para decidir qué publicar.
...
La primera implementación mostró que el experimento de UI también era un problema de sistemas. La herramienta necesitaba:
...
El caché fue el requisito que más cambió el diseño.
...
Lo reutilizable fue el sistema de testing. El equipo podía pasar de una hipótesis a la entrega ponderada, validar analytics, mantener el mismo tratamiento durante SSR y separar el caché por variante. Después de decidir, podía retirar el experimento. Así, los desacuerdos de producto tenían datos en vez de resolverse por la opinión con más peso.
...
El control ganador no fue el único resultado de Peelie. El equipo quedó con una forma repetible de convertir un desacuerdo de producto en una pregunta verificable, probar la medición y aceptar la respuesta aunque indicara conservar lo que ya teníamos.
