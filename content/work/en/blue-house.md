...
description: How I built Blue House to collect Argentine dollar rates, store their history, and serve them through an API and dashboard.
project: Blue House
...
Blue House is a personal project for collecting, querying, and presenting Argentine peso exchange rates against the US dollar. I built it because no available source gave me one dependable interface for current quotes and historical changes. The MVP combines live and historical ingestion, PostgreSQL storage, a documented API, and a responsive Nuxt dashboard. It proves that the complete system works, but not that it can yet operate as a mature public service.
...
For most visitors, the first useful comparison is official versus blue. Other rates provide context, but giving all seven equal visual weight would make the product harder to scan. Blue House gives official and blue rates priority instead of displaying an undifferentiated table of numbers.
...
A wrapper around a third-party endpoint would not solve the problem. I had to normalize two providers without erasing provenance, prevent scheduled jobs from duplicating or corrupting data, define dates in Buenos Aires time, and build an interface that worked on a phone.

## Exploration and proof of concept

I first tested whether I could turn the provider data into a consistent timeline. The initial poller fetched live observations, validated them with Zod, generated deterministic fingerprints, and stored them in PostgreSQL. I then added one part at a time:
...
This work exposed data and operational problems before I spent time on accounts, access control, or monetization.
...
### Application guards or database coordination
...
I also built browsable documentation and an endpoint tester that generates examples and displays status, duration, and formatted responses. Users can inspect the API without reading the source code.

`[Image: interactive API documentation with a request and formatted response]`

## Designing the public view

The dashboard resembles an exchange bulletin rather than a trading terminal. Official and blue quotes receive the largest cards, while the other five categories form a secondary group. A seven-series chart shows movement over selectable periods, and the interface shows when data was last updated.
...
These changes set a clear limit for the MVP. It preserves and explains source data. It will add interpretation only after I can validate the behavior.

## Reflection and next steps

Blue House forced me to work through each part of a small data product, including provider adapters, database migrations, scheduled jobs, service calculations, container builds, API documentation, and the visual design.

The next step is to prove that it operates reliably, not to add features. That requires repeatable migrations, real-database deduplication and rollback tests, both container builds, scheduler and backup checks, and deployed freshness and reliability measurements.
