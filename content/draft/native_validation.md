...
You can use form controls' `.validationMessage` property to get the native validation message, and display it in the UI the way you like it.

May save you a lot of manually implemented logic too :)
...
inputEl.addEventListner('input', (event) => {
  let message = ''
...
inputEl.addEventListner('input', (event) => {
  msgEl.innerText = inputEl.validationMessage
})
```
