...
import type { AI_MODEL_REQUEST_CLIENT, AI_MODEL_RESPONSE, MODEL_TYPES, ChatMessage } from '@/types'
import { allowedModels } from '@/types'
import { getResponseMsg, getNewId } from '@/utils'
    ...
    <section class="flex flex-col p-4">
        ...
        <div class="flex gap-2" @keypress.enter="sendMessage">
            ...
            <button type="submit"
                ...
                class="inline-flex items-center gap-x-2 rounded-lg bg-blue-600 px-3.5 py-2.5 text-center text-sm font-medium text-slate-50 hover:bg-blue-800 focus:(outline-none ring ring-blue-300) disabled:(bg-blue-200 text-slate-400 cursor-not-allowed hover:bg-blue-200 hover:text-slate-400)"
                @click="sendMessage"
                >
                <i v-if="isResponding" class="i-svg-spinners:180-ring-with-bg"></i>
                    <i v-else class="i-ri:send-plane-fill"></i>
            </button>
        </div>
    </section>
</template>
